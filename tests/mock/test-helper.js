...
module.exports = {
     ...
     */
    initTest: function(express, dataStore, fns, api, test) {
        switch (arguments.length) {
            ...
            case 3:
                test = arguments[2];
                api = arguments[1];
                if (arguments[0] instanceof env.swagger.DataStore) {
        ...
        var supertest = env.supertest(express.app || express);

        env.swagger(api, express, function(err, middleware) {
            express.use(
                middleware.metadata(),
                middleware.CORS(),
