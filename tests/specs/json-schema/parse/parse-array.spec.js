...
describe('JSON Schema - parse array params', function() {
  ...
  it('should return an empty array if there is no default value and the specified value is blank',
    function(done) {
      var schema = {
        type: 'array',
        items: {
      ...
      express.use('/api/test', helper.spy(function(req, res, next) {
        expect(req.header('Test')).to.deep.equal([]);
      }));
    }
  );
