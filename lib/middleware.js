...
function Middleware(sharedRouter) {
  ...
  this.init = function(swagger, callback) {
    ...
    var parserInstance = new SwaggerParser();

    //Allow for reference to the swaggerParser instance from the promise.resolve block
    //to access the meta data that gets processed
    var me = parserInstance;

    //Need to retrieve the Swagger API and metadata from the Swagger .yaml or .json file.
    parserInstance.dereference(swagger)
      .then(function(api) {
        context.api = api;
        context.metadata.files = Object.keys(me.$refs._$refs);
