...
function Middleware(sharedRouter) {
  ...
  this.init = function(swagger, callback) {
    ...
    context.metadata.baseDir = path.dirname(swagger);
    ...
    swaggerParser.dereference(swagger, function(err, api) {
      if (err) {
        util.warn(err);
      }
      ...
      swaggerParser.resolve(swagger, function(err, metadata) {
        context.metadata.files = Object.keys(metadata._$refs);
        context.emit('change');

        if (_.isFunction(callback)) {
          callback(err, self, context.api, context.metadata);
        }
      });
    });
    ...
    //
    //    if (_.isFunction(callback)) {
    //      callback(null, self, context.api, context.metadata);
    //    }
    //  })
    //  .catch(function(err) {
    //    util.warn(err);
    ...
    //    context.emit('change');
    //
    //    //swagger variable should only be a file or an object
    //    if (_.isFunction(swagger)) {
    //      throw new Error('Expected a Swagger file or object');
    //    }
    //
