...
function isParamContentLengthRequiredAndInHeader(param) {
  return param.required && param.in === 'header' && param.name.toLowerCase() === 'content-length';
}

function isContentLengthValueMissing(value) {
...
function parseParameter(param, value, schema) {
  if (isParamContentLengthRequiredAndInHeader(param) && isContentLengthValueMissing(value)) {
    // Special case for the Content-Length header.  It has it's own HTTP error code.
    errCode = 411; // (Length Required)

    //noinspection ExceptionCaughtLocallyJS
    throw ono({status: errCode}, 'Missing required content-length value');
  }
