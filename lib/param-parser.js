    ...
    params.forEach(function(param) {
      ...
      switch (param.in) {
        case 'query':
          ...
          req.query[param.name] = parseParameter(param, req.query[param.name], schema);
          break;
        case 'header':
          // NOTE: req.headers properties are always lower-cased
          util.debug('    Parsing the "%s" header parameter', param.name);
          req.headers[param.name.toLowerCase()] = parseParameter(param, req.header(param.name), schema);
          break;
      }
    });
  }
  ...
  getParams(req).forEach(function(param) {
    ...
    if (param.in === 'formData') {
      ...
      if (param.type === 'file') {
        ...
        req.files[param.name] = parseParameter(param, req.files[param.name], schema);
      } else {
        // Parse the body parameter
        req.body[param.name] = parseParameter(param, req.body[param.name], schema);
      }
    }
  });
