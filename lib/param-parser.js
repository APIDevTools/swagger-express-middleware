...
function paramParser (options) {
  ...
  let parseCookieParams = cookieParser(options.cookie.secret, options.cookie);
  ...
  /**
   * Parses all OpenAPI parameters in the query string and headers
   */
      ...
      params.forEach((param) => {
        ...
        switch (param.in) {
          ...
          case "header":
            ...
            break;
          case "cookie":
            // TODO - Only parse cookies that are defined in the OpenAPI definition
            parseCookieParams(req, res, next);
            break;
        }
      });
    }
...
/**
 * Parses all OpenAPI parameters in the form data.
 */
...
/**
 * Parses the OpenAPI "body" parameter.
 */
...
/**
 * Returns an array of the `req.openapi.params` properties.
 *
 * @returns {object[]}
 */
function getParams (req) {
  if (req.openapi && req.openapi.params) {
    return req.openapi.params;
  }
  return [];
}
