...
function parsePrimitive (parseInfo) {
  ...
  switch (schema.type) {
    ...
    default:
      throw parseError("is not a primitive type", new ParseInfo({ ...parseInfo, value: schema.type }));
  }
}
...
function parse (parseFn, parseInfo) {
  ...
  if (value === undefined || value === "") {
    if (typeof schema.default === "string") {
      // The default value might be a plain string, or it might be a Date, Base64 string, or binary.
      // So parse it just like we'd parse a parameter. If parsing fails, throw an HTTP 500 (Server Error)
      return parsePrimitive(new ParseInfo({ ...parseInfo, value: schema.default, status: 500 }));
    }
    else {
      // For all other types, the default value in the schema is already the correct type
      // (number, boolean, array, object), so no parsing is needed.
...
function parseError (suffix, parseInfo) {
  let { param, value, path, status } = parseInfo;
  ...
  let message = `The "${param.name}" header parameter is invalid.`;

  if (path !== param.name) {
    message += ` Error at ${path}.`;
  }

  return ono.syntax({ status }, `${message} "${value}" ${suffix}.`);
}
