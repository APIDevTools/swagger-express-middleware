...
function http404(req, res, next) {
    if (req.swagger && req.swagger.api && !req.swagger.path) {
        util.debug('Client requested path "%s", which is not defined in the Swagger API. Returning HTTP 404 (Not Found)', req.path);
...
function http405(req, res, next) {
    if (req.swagger && req.swagger.path && !req.swagger.operation) {
        util.debug(
            'Client attempted a %s operation on "%s", which is not allowed by the Swagger API. Returning HTTP 405 (Method Not Allowed)',
            req.method, req.path
        );
...
function http406(req, res, next) {
    if (util.isSwaggerRequest(req)) {
        ...
        if (produces.length > 0) {
            util.debug('Validating Accepts header (%s)', req.get('Accepts'));
...
function http413(req, res, next) {
    if (util.isSwaggerRequest(req)) {
        ...
        if (!bodyAllowed) {
            ...
            // NOTE: A Content-Length of zero is NOT the same as having no body 
            // (e.g. a zero-byte file attachment, an empty string, etc.)
            if (!isNaN(length)) {
                util.debug(
