 ...
 * In Swagger 2.0, this is simply the `basePath` property.
 * In OpenAPI 3.0, this is the path portion of the `servers` URL.
 *
 * @param   {object}   api
 * @returns {string|undefined}
 */
exports.getBasePath = function (api) {
  if (api.swagger) {
    return api.basePath;
  }
  ...
  if (Array.isArray(api.servers) && api.servers.length > 0) {
    ...
    let serverUrl = server.url.replace(/\{([^}]+)\}/g, (match, variableName) => {
