...
function JsonSchema(schema) {
  if (!schema) {
    throw ono('Missing JSON schema');
  }
  if (schema.type !== undefined && dataTypes.indexOf(schema.type) === -1) {
    throw ono('Invalid JSON schema type: %s', schema.type);
  }
...
function jsonValidate(schema, value, propPath) {
  if (tv4.validate(value, schema)) {
    ...
    throw ono({status: 400}, tv4.error, 'JSON Schema validation error. \nData path: "%s" \nSchema path: "%s"',
...
function parseArrayItems(schema, array, propPath) {
  ...
  for (var i = 0; i < array.length; i++) {
    ...
    try {
      ...
      throw ono({status: 400}, e, 'Unable to parse %s item at index %d (%j).', propPath || 'array', i, item);
    }
  }
}
...
function parseFile(schema, value, propPath) {
  ...
  if (schema.minLength) {
    ...
    if (isNaN(minLength)) {
      throw ono('The "minLength" value in the Swagger API is invalid (%j)', schema.minLength);
    }
  ...
  if (schema.maxLength) {
    ...
    if (isNaN(maxLength)) {
      throw ono('The "maxLength" value in the Swagger API is invalid (%j)', schema.maxLength);
    }
...
function parseDate(schema, value, propPath) {
  ...
  if (schema.minimum) {
    ...
    if (isNaN(minDate.getTime())) {
      throw ono('The "minimum" value in the Swagger API is invalid (%j)', schema.minimum);
    }
  ...
  if (schema.maximum) {
    ...
    if (isNaN(maxDate.getTime())) {
      throw ono('The "maximum" value in the Swagger API is invalid (%j)', schema.maximum);
    }
