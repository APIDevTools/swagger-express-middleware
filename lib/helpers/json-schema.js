...
function JsonSchema(schema) {
    ...
    if (dataTypes.indexOf(schema.type) === -1) {
...
function serializeInteger(schema, value, propPath) {
    return parseInt(getValueToValidate(schema, value));
}
...
function serializeNumber(schema, value, propPath) {
    return parseFloat(getValueToValidate(schema, value));
}
...
function sampleNumber(schema) {
    var min, max;
    if (schema.minimum !== undefined) {
        min = parseFloat(schema.minimum) + (schema.exclusiveMinimum ? Number.EPSILON : 0);
    }
    else {
    ...
    if (schema.maximum !== undefined) {
        max = parseFloat(schema.maximum) - (schema.exclusiveMaximum ? Number.EPSILON : 0);
    }
    else {
...
function serializeString(schema, value, propPath) {
    return _(getValueToValidate(schema, value)).toString();
}
    ...
    if (value) {
        _.forEach(schema.properties, function(prop, propName) {
            if (value[propName] !== undefined) {
                var propSchema = new JsonSchema(prop);
                var fullPropPath = propPath ? propPath + '.' + propName : propName;
                value[propName] = propSchema.serialize(value[propName], fullPropPath);
            }
        });
    }
