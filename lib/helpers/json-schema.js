...
function parseFile (schema, value, propPath) {
  ...
  if (schema.minLength) {
    ...
    if (isNaN(minLength)) {
      throw ono({ status: 500 }, 'The "minLength" value in the OpenAPI definition is invalid (%j)', schema.minLength);
    }
  ...
  if (schema.maxLength) {
    ...
    if (isNaN(maxLength)) {
      throw ono({ status: 500 }, 'The "maxLength" value in the OpenAPI definition is invalid (%j)', schema.maxLength);
    }
...
function parseDate (schema, value, propPath) {
  ...
  if (schema.minimum) {
    ...
    if (isNaN(minDate.getTime())) {
      throw ono({ status: 500 }, 'The "minimum" value in the OpenAPI definition is invalid (%j)', schema.minimum);
    }
  ...
  if (schema.maximum) {
    ...
    if (isNaN(maxDate.getTime())) {
      throw ono({ status: 500 }, 'The "maximum" value in the OpenAPI definition is invalid (%j)', schema.maximum);
    }
