...
function fileServer(context, router, options) {
    ...
    function serveDereferencedSwaggerFile(req, res, next) {
        if (req.method === 'GET' || req.method === 'HEAD') {
            ...
            if (options.useBasePath && context.api && context.api.basePath) {
                optPath = context.api.basePath + options.dereferencedJsonPath;
            }
            else {
                optPath = options.dereferencedJsonPath;
            }

            optPath = util.normalizePath(optPath, router);
            reqPath = util.normalizePath(req.path, router);
    ...
    function serveRawSwaggerFiles(req, res, next) {
        if (req.method === 'GET' || req.method === 'HEAD') {
            ...
            if (options.useBasePath && context.api && context.api.basePath) {
                optPath = context.api.basePath + options.rawFilesPath;
            }
            else {
                optPath = options.rawFilesPath;
            }

            optPath = util.normalizePath(optPath, router);
            reqPath = util.normalizePath(req.path, router);

            if (_.startsWith(reqPath, optPath) && context.metadata) {
                // Get the normalized, relative path of the requested file
                var relativePath = req.path.substring(optPath.length);
                if (_.startsWith(relativePath, '/')) {
                    relativePath = relativePath.substring(1);
                }
                relativePath = util.normalizePath(relativePath, router);
