...
function queryResource(req, res, next, dataStore) {
    ...
    dataStore.getResource(resource, function(err, result) {
        if (err) {
            next(err);
        }
        else if (!result) {
...
function getDefaultValue(res) {
    ...
    else if (res.swagger.response.schema) {
        return res.swagger.response.schema.default || res.swagger.response.schema.example;
    }
}
