  ...
  if (resources.length > 0) {
    ...
    if (queryParams.length > 0) {
      ...
      queryParams.forEach(function(param) {
        var reqParam = req.query[param.name];
        if (reqParam !== undefined) {
          if (param.name === 'offset' && param.type === 'integer') {
            offset = reqParam !== undefined ? reqParam : param.default;
          }
          else if (param.name === 'limit' && param.type === 'integer') {
            limit = reqParam !== undefined ? reqParam : param.default;
          }
          else {
            setDeepProperty(filterCriteria.data, param.name, reqParam);
          }
        }
      });
      ...
      // Filter paginated resources
      if (offset !== undefined || limit !== undefined) {
          util.debug('Filtering pagination offset %d and limit %d', offset || 0, limit);
          resources = resources.slice(offset || 0, limit !== undefined ? limit : resources.length);
      }
    }
  }
