 ...
 * A unique URL is generated for each new resource, based on the schema definition in the OpenAPI definition,
...
function mergeCollection (req, res, next, dataStore) {
  ...
  res.openapi.location = _.isArray(req.body) ? collection : resources[0].toString();

  // Save/Update the resources
  util.debug("Saving data at %s", res.openapi.location);
...
function createResources (req) {
  let resources = [],
      body = req.body,
      schema = util.getRequestSchema(req.openapi.path, req.openapi.operation);
...
function sendResponse (req, res, next, dataStore) {
  return function (err, resources) {
    if (!err) {
      util.debug("%s successfully updated", res.openapi.location);

      if (resources.length > 0) {
        // Determine the max "modifiedOn" date of the resources
        res.openapi.lastModified = _.max(resources, "modifiedOn").modifiedOn;
      }
      else {
