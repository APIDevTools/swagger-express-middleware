...
function mergeCollection(req, res, next, dataStore) {
    ...
    dataStore.saveCollection(collection, resources, sendResponse(req, res, next, dataStore, resources));
}
...
function overwriteCollection(req, res, next, dataStore) {
    ...
    dataStore.deleteCollection(collection, resources, function(err) {
        if (err) {
            next(err);
        }
        else {
...
/**
 * Returns the JSON schema for the request's resources.
 *
 * @param   {Request}  req
 * @returns {object}
 */
function getResourceSchema(req) {
    // If there's a "body" parameter, then use its schema
    var bodyParam = _.find(req.swagger.params, {in: 'body'});
    if (bodyParam) {
        ...
        _.where(req.swagger.params, {in: 'formData'}).forEach(function(param) {
...
function getResourceNameByName(data, schema) {
    ...
    var nameProperties = ['id', 'key', 'code', 'number', 'num', 'nbr', 'username', 'name'];
 ...
 * @param   {function}      next
 * @param   {DataStore}     dataStore
 * @param   {Resource[]}    editedResources
 */
function sendResponse(req, res, next, dataStore, editedResources) {
    return function(err, allResources) {
        if (!err) {
            util.debug('%s successfully updated', res.swagger.location);

            if (allResources.length > 0) {
                // Determine the max "modifiedOn" date of the remaining items
                res.swagger.lastModified = _.max(allResources, 'modifiedOn').modifiedOn;
            }
            else {
            ...
            allResources = _.pluck(allResources, 'data');
            editedResources = _.pluck(editedResources, 'data');
        }
        ...
        else if (res.swagger.response.schema && res.swagger.response.schema.type !== 'array') {
            // Response body is a single value, so only return the first item that was edited
            res.body = _.first(editedResources);
            next();
        }
        else {
            // Response body is the entire collection (edited and old)
            res.body = allResources;
            next();
        }
    };
}
